...
for nepisode in range(num_episodes):
    ...
    for nstep in range(steps_per_episode):
        ...
        image_l = full_state['observation']['sensors']['color']['data']
        image_r = full_state['observation']['sensors']['right']['data']
        depth = full_state['observation']['sensors']['depth']['data']
        # print("Image shape", image.shape, ", Depth shape", depth.shape)

        image_l = np.reshape(image_l, (image_l.shape[1], image_l.shape[0], -1))
        image_r = np.reshape(image_r, (image_r.shape[1], image_r.shape[0], -1))
        depth = np.reshape(depth, (depth.shape[1], depth.shape[0], -1))

        # print("Image shape", image.shape, ", Depth shape", depth.shape)
        image_l_filename = os.path.join(episode_folder, "{:03}_color_l.jpg".format(nstep))
        image_r_filename = os.path.join(episode_folder, "{:03}_color_r.jpg".format(nstep))
        depth_filename = os.path.join(episode_folder, "{:03}_depth.png".format(nstep))
        imageio.imwrite(image_l_filename, image_l[:,:,:3])
        imageio.imwrite(image_r_filename, image_r[:,:,:3])
